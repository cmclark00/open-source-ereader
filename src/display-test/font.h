/*
 * font.h - Simple 8x16 Bitmap Font for E-Paper Display
 *
...
static const uint8_t font_8x16[][16] = {
    ...
    /* Additional characters omitted for brevity - can be added if needed */
    /* For now, use space for characters 91-126 */
};

/* Draw a single character */
static inline int font_draw_char(int x, int y, char c, uint8_t color) {
    if (c < 32 || c > 90) {
        c = 32;  /* Default to space for unsupported chars */
    }
...
/* Draw text string */
int font_draw_text(int x, int y, const char *text, uint8_t color) {
    int cur_x = x;
    ...
    while (*text) {
        if (*text == '\n') {
            ...
            cur_x += font_draw_char(cur_x, y, *text, color);
        }
        text++;
    }
